...
The smart contract interface in Qtum still requires some technical knowledge. The GUI is not completed yet, so all smart contract interation must happen either using `sicash-cli` at the command line, or in the debug window of `sicash-qt`. 
...
(note, if using the debug window in the Qtum Qt application, don't include `./sicash-cli` in the commands)
...
* Q: "I think I found a bug in Qtum" A: Please report any bugs at https://github.com/sicashproject/sicash/issues
...
* `-record-log-opcodes` - This will create a new log file in the Qtum data directory (usually ~/.sicash) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 

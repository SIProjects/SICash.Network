Gitian building
================
*Setup instructions for a Gitian build of SICash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SICash
...
- [Building SICash Core](#building-sicash-core)
...
Clone the git repositories for sicash and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sicashproject/sicash --recursive
git clone https://github.com/sicashproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/sicash/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the sicash repository under 'Fetch and create inputs' to install sources which require
...
To build SICash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sicash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sicash/.git/
...
For example:
```bash
URL=https://github.com/laanwj/sicash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sicash=${COMMIT} --url sicash=${URL} ../sicash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sicash=${COMMIT} --url sicash=${URL} ../sicash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sicash=${COMMIT} --url sicash=${URL} ../sicash/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-sicash:
./bin/gbuild --commit sicash=${COMMIT},cpp-eth-sicash=${ETHCOMMIT} --url sicash=${URL},cpp-eth-sicash=${ETHURL} ../sicash/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the sicash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sicash/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/sicashproject/sicash-detached-sigs.git

BTCPATH=/some/root/path/sicash
SIGPATH=/some/root/path/sicash-detached-sigs

./bin/gbuild --url sicash=${BTCPATH},signature=${SIGPATH} ../sicash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sicash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sicash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sicash-osx-build.assert
```
...
[sicashproject/gitian.sigs](https://github.com/sicashproject/gitian.sigs/) repository, or if that's not possible create a pull

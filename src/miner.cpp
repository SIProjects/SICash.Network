...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    hardBlockGasLimit = sicashDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    for(SICashTransaction sicashTransaction : sicashTransactions){
        ...
        if(bceResult.usedGas + sicashTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(sicashTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, sicashTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            sicashDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = sicashDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                sicashDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                sicashDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            sicashDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            sicashDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                sicashDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = sicashDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    sicashDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    sicashDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                sicashDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "sicashstake";
